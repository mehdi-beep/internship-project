...
|---|---|---|
| Roles | `seed_roles()` | 3 (technician, chef_technicien, admin_supervisor) |
| Users | `seed_users()` | 14 (10 technicians `tech01`–`tech10`, 2 chefs `chef01`–`chef02`, 2 admins `admin01`–`admin02`) |
...
Every seeded technician/chef/admin shares the same demo password: **`Password123!`**.
...
```python
# Line 156 — technicians (currently range(1, 11) = 10 technicians)
for i in range(1, 11):
    ...
# Line 161 — chefs (currently range(1, 3) = 2 chefs)
for i in range(1, 3):
    ...
# Line 166 — admins (currently range(1, 3) = 2 admins)
for i in range(1, 3):
    ...
```
...
Edit line 178, `for _ in range(22):` inside `seed_clients_and_sites()`. Each client
...
Edit the `MOROCCAN_CITIES` list at **line 55** of `seed.py`:
```python
MOROCCAN_CITIES = [
...
Edit line 205, `for _ in range(28):` (contracts) inside `seed_contracts_and_projects()`.
...
Edit the `TRAVAUX_CATALOG` list at **line 61** of `seed.py` — it's a list of
...
Edit line 286, `target_count = 650`, inside `seed_interventions()`. Raising this
...
Same pattern: line 484 (`for _ in range(220):`) for planning, line 536
