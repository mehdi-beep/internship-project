...
def create(db: Session, *, user_id: int, title: str, message: str, related_intervention_id: int | None = None) -> Notification:
    notification = Notification(
        user_id=user_id, title=title, message=message, related_intervention_id=related_intervention_id
    )
    db.add(notification)
    db.commit()
