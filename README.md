...
**Live in production as of 2026-09-08.** Email is configured and confirmed working end-to-end on `bi.cortextechnology.ma` (a dedicated `noreply@bi.cortextechnology.ma` mailbox, SMTP over TLS on port 587). Getting there required two rounds of DNS/authentication debugging beyond the SMTP credentials themselves: the `bi.` subdomain initially had no SPF record at all, so the parent domain's strict DMARC policy (`p=reject; sp=reject; aspf=s`) silently discarded every message even though the SMTP conversation itself completed successfully; after adding an SPF record, delivery still failed because that record authorized the domain's website IP rather than its actual, different mail-server IP — found via cPanel's Email Deliverability tool, not guessing. Installing the correct SPF record (the real mail-server IP) plus an explicit subdomain DMARC policy resolved it. A real assignment-notification email was independently confirmed received. WhatsApp remains unconfigured (requires a Meta Business account and template approval — a manual, external setup step, not a code gap).

Also since Task 9: a per-account **Do Not Disturb** toggle (`users.dnd_enabled`, set via `PATCH /api/notifications/dnd`) suppresses only the external email/WhatsApp copy of a notification at creation time — in-app notifications are unaffected and still arrive (unread) regardless. Because dispatch happens exactly once, synchronously, at creation with no retry path anywhere in the codebase, a notification created while DND is on can never be emailed later either, even after DND is turned back off. Independently confirmed working in production. Separately, external dispatch coverage was widened from the original 5 assignment-related notification types to all 9 — submission, both approval-workflow notifications, and rejection now email too, not just assignment/reassignment/cancellation.
