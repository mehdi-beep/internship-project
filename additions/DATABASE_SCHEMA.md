...
### role_name
`technician`, `chef_technicien`, `admin_supervisor` (Ch.36 — exactly 3 roles, no Administrator), plus `display` (Task 3 — a strictly read-only hallway-calendar account, added post-launch; not part of the original SRS's 3-role model)
...
### point_rules (Task 2 — post-launch, not part of the original 154-chapter SRS)
| Column | Type | Notes |
|---|---|---|
| id | SERIAL PK | |
| start_time | TIME NOT NULL | |
| end_time | TIME NOT NULL | `end_time <= start_time` represents a window crossing midnight (e.g. 22:00–00:00) |
| points | INTEGER NOT NULL | positive, zero, or negative |
| active | BOOLEAN NOT NULL DEFAULT TRUE | inactive rules are excluded from both scoring and overlap-conflict checks |
| created_at / updated_at | TIMESTAMPTZ | |

No foreign key references this table from anywhere else — `interventions.points_earned` stores the computed integer value at submission time, never a reference back to the rule that produced it, which is what makes editing/deleting a rule safe for history (see `business_logic_service.calculate_points()`). Seeded with 3 default rows reproducing the original Ch.28 hardcoded windows exactly; see [`SYNTHETIC_DATABASE.md`](SYNTHETIC_DATABASE.md).
...
### Permanent deletion (Task 5, post-launch)

In addition to the soft-delete behaviour above, the Administrator can
**permanently delete** users, clients, client sites, contracts, projects and
travaux — but *only* when nothing references the record. `app/services/deletion_service.py`
counts every inbound foreign key first and refuses with `409` otherwise,
naming each blocker. This preserves the Ch.49 `ON DELETE RESTRICT` guarantee
and Rule 9: no cascade ever reaches `interventions`, `approval_history` or
`audit_log`, so historical data cannot be destroyed by an administrative
action. In practice any record with real history is undeletable by design and
must be deactivated/archived instead.

`point_rules` (Task 2) remains the one table with genuinely unconditional
hard deletes, because nothing holds a foreign key to it.
