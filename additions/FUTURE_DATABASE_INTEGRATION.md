...
| **`backend/app/database/seed.py`** | Would simply stop being run against the real database for the *demo-data generator* part of it (clients, interventions, planning, etc.) — that part is invoked manually (`python -m app.database.seed`) or automatically only as an empty-database safety net (`main.py`'s startup hook), and its own idempotency guard (checking whether the `roles` table is empty) means it immediately no-ops against an already-populated database, by design. **One nuance, post-Tasks 2/3**: `seed_point_rules()` and `seed_display_role_and_account()` now run *unconditionally* at the top of `run()`, independently of that same guard, specifically so a company's real (non-demo) database still gets 3 usable default point rules and a `display` role/account backfilled on first boot rather than starting with an empty, unusable `point_rules` table — this is intentional and desired even against real production data, not something to disable. |
...
15-table schema (14 from the original SRS-driven build, plus `point_rules` from the
post-launch Task 2 configurable-point-rules work) regardless of which physical
database instance backs it. Two realistic *exceptions* worth naming honestly, though:
...
4. **Populate reference/master data first**, in dependency order (this mirrors exactly
   ...
   three employee-facing roles — `display` is a shared kiosk login, not an employee
   role, see Section 10) → Clients → Client Sites → Contracts / Projects → the
   Travaux catalog.
...
- **Verify role assignment carefully during the user-import step** — a technician
  ...
  cosmetic. The post-launch `display` role (Task 3) adds a distinct version of this
  same risk: it is a shared kiosk/device login with no real employee behind it, so it
  should never be assigned to an actual person during a real-employee import, and
  conversely a real employee should never be assigned `display` by mistake — either
  mix-up would either lock a real person out of their intended role or give a shared
  hallway device an identity it shouldn't have.
