...
Repository layer (app/repositories/*.py) — all database query construction
        │
        ▼
Model layer (app/models/*.py)           — SQLAlchemy ORM entities (14 tables)
```
...
14 tables, centered on one core entity (`interventions`), with two genuine
...
`DELETE` against them. See `DATABASE_SCHEMA.md` for the full column-by-column
breakdown; the full entity list and relationships are also covered in Section 8 of
this document at a summary level.
...
│   ├── models/          SQLAlchemy ORM entities — one file per table, 14 total.
...
├── tests/               The full pytest suite (122 tests), one file per feature area,
...
│                    between. Includes two sub-directories: `admin/` (the six
│                    reference-data management screens, admin_supervisor-only) and
│                    `dashboards/` (the per-role dashboard content components).
